...
(function () {
  ...
  /* ======================= 1. НАСТРОЙКИ ======================= */
  var APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbxELHq6HYpNNLcCz4BrfUFNx4AintXr-mN0dK_jg0qrBM8axV-5fEmmZ4fdflEaVoBGmQ/exec';
  ...
  function isConfigured() {
    return APPS_SCRIPT_URL && APPS_SCRIPT_URL.trim().indexOf('ВСТАВЬТЕ_СЮДА') === -1;
  }
  ...
  function fetchGuest(cb) {
    if (!guestId) { console.log('[wedding] нет параметра ?guest= в ссылке — персонализация выключена'); cb(null); return; }
    if (!isConfigured()) { console.log('[wedding] APPS_SCRIPT_URL не настроен в assets/wedding.js — см. SETUP.md шаги 1-3'); cb(null); return; }
    console.log('[wedding] запрашиваю данные гостя:', guestId);
    fetch(APPS_SCRIPT_URL.trim() + '?guest=' + encodeURIComponent(guestId))
      .then(function (r) { return r.json(); })
      .then(function (data) {
        console.log('[wedding] ответ от Apps Script:', data);
        cb(data);
      })
      .catch(function (err) {
        console.error('[wedding] ошибка запроса к Apps Script:', err);
        cb(null);
      });
  }
  ...
  function renderPersonCard(personId) {
    ...
    card.appendChild(textField('Фамилия', data.last_name, function (v) { data.last_name = v; }, '', 'rsvp-field-tight'));
    card.appendChild(textField('Имя', data.first_name, function (v) { data.first_name = v; }, '', 'rsvp-field-tight'));
    ...
    function renderRest() {
      ...
      secondWrap.appendChild(radioGroup('stay_' + personId, [{ value: '1', label: 'Да' }, { value: '0', label: 'Нет' }], data.stay_second_day, function (v) { data.stay_second_day = v; }));
  ...
  function showLoadingOverlay() {
    var overlay = el('div', { class: 'rsvp-thanks-overlay', id: 'rsvp-loading-overlay' }, [
      el('div', { class: 'rsvp-thanks-box' }, [
        el('div', { class: 'rsvp-loading-text', text: 'Пожалуйста, подождите' }),
        el('img', { class: 'rsvp-loading-birds', src: 'pics/birds_start.png', alt: '' })
      ])
    ]);
  ...
  function hideLoadingOverlay() {
    var overlay = qs('#rsvp-loading-overlay');
